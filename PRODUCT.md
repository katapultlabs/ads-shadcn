...
- **Brand-driven, not hardcoded.** Visual identity flows from theme tokens (`client-theme.json` / `ads.*` semantic tokens), so the explorer restyles with the brand.
